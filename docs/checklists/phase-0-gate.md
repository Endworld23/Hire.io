...
☐ `docs/roadmap.md` explicitly enforces B2B-first sequencing and marketplace gating
☐ `docs/architecture.md` reflects the actual system boundaries and data flows
...
☐ A consolidated schema migration exists and matches production intent
...
☐ Client role cannot access PII fields (name, email, phone, resume raw text)
...
☐ AI calls are logged (model, template ID, timestamp; no raw content)
